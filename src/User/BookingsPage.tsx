...
interface Booking {
  id: number | string;
  user_id: number;        // ← now returned by backend
  user: string;
  court: string;
  timeslot: string;
  status: string;
...
const statusConfig = (status?: string) => {
  switch ((status || "").toLowerCase()) {
    ...
    case "cancelled": return { dot: "bg-slate-500",  badge: "bg-slate-500/10 text-slate-400 border-slate-500/20"  };
    default:          return { dot: "bg-blue-500",   badge: "bg-blue-500/10 text-blue-400 border-blue-500/20"   };
  }
};

// ✅ Read userId from JWT — matches what authController.js signs
const getTokenPayload = (): { userId: number; name: string; role: string } | null => {
  try {
...
const BookingsPage: React.FC = () => {
  ...
  const currentUserId = tokenPayload?.userId ?? null;
  const currentName   = tokenPayload?.name   ?? null;
    ...
    socket.on("bookingsUpdated", (updated: Booking[]) => {
      setBookings(prev => {
        ...
        if (newIds.length > 0) {
          flashRows(newIds);
          addToastRef.current(
            newIds.length === 1 ? "A booking was just updated" : `${newIds.length} bookings updated`,
            "info"
          );
        }
        return updated;
  ...
  // ── Filter — match by user_id (reliable) with name fallback ───────────────

  const myBookings = bookings.filter(b => {
    if (currentUserId && b.user_id) return b.user_id === currentUserId;
    // Fallback: name match (for tokens issued before the fix)
    if (currentName) return b.user?.toLowerCase() === currentName.toLowerCase();
    return false;
  });
      ...
      <div className="min-h-screen bg-[#0d0d1a] font-mono-dm px-8 py-6">
        ...
        >
          ...
          {!loading && displayed.map((b, i) => {
            ...
            // ✅ Match by user_id (primary), fall back to name
            const isMine = currentUserId
              ? b.user_id === currentUserId
              : b.user?.toLowerCase() === currentName?.toLowerCase();
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    {isMine && <span className="text-[8px] text-orange-400 tracking-widest">YOU</span>}
                  </div>
                </div>
                ...
                <div>
                  ...
                  {isFlashing && <Zap size={8} className="inline ml-1 text-orange-400" />}
                </div>
